...
async function runPipeline(id) {
  try {
    await store.update(id, (r) => ({ ...r, status: 'validating' }));
    await wait(STAGE_PAUSE_MS);

    const report = await store.getById(id);
    if (!report) return;

    const a1 = await agent1.validate(report);

    if (!a1.valid) {
      await store.update(id, (r) => ({ ...r, status: 'rejected', agent1: a1 }));
      return;
    }
    await store.update(id, (r) => ({ ...r, status: 'validated', agent1: a1 }));

    await wait(STAGE_PAUSE_MS);
    await store.update(id, (r) => ({ ...r, status: 'patching' }));
    const a2 = await agent2.generatePatch(a1, report);

    if (!a2.can_fix) {
      await store.update(id, (r) => ({ ...r, status: 'needs_review', agent2: a2 }));
      return;
    }
    await store.update(id, (r) => ({ ...r, status: 'patched', agent2: a2 }));

    await wait(STAGE_PAUSE_MS);
    await store.update(id, (r) => ({ ...r, status: 'deploying' }));
    const a3 = await agent3.deploy(a2, id, report);

    if (!a3.applied) {
      await store.update(id, (r) => ({ ...r, status: 'failed', agent3: a3 }));
      return;
    }
    if (a3.pushAttempted && !a3.pushed) {
      await store.update(id, (r) => ({ ...r, status: 'failed', agent3: a3, error: a3.pushLog }));
      return;
    }

    await store.update(id, (r) => ({ ...r, status: 'deployed', agent3: a3 }));
  } catch (err) {
    console.error(`[pipeline] Unhandled error processing ${id}:`, err);
    await store.update(id, (r) => ({ ...r, status: 'failed', error: err.message }));
  }
}
