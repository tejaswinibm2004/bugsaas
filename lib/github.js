...
async function listUserRepos(owner = process.env.GITHUB_OWNER) {
  ...
  // Try user/repos first for authenticated token, fallback to users/{owner}/repos
  const urls = [
    'https://api.github.com/user/repos?per_page=100&sort=updated',
    owner ? `https://api.github.com/users/${owner}/repos?per_page=100&sort=updated` : null,
  ].filter(Boolean);
...
async function resolveRepoForReport(report = {}, owner = process.env.GITHUB_OWNER) {
  ...
  // 1. If explicit repo was passed (e.g. report.repo or data-repo="test")
  if (report.repo) {
    const explicitLower = report.repo.toLowerCase();
    const match = userRepos.find((r) => r.name.toLowerCase() === explicitLower);
    if (match) return match.name;
    return report.repo; // Fallback to provided string
  }

  // 2. Derive keywords from report.url (e.g. https://test-c4dz.onrender.com/ -> "test")
  if (report.url) {
    try {
...
async function getFileFromGitHub(filePath, owner = process.env.GITHUB_OWNER, repo = process.env.GITHUB_REPO || 'test', branch = process.env.GITHUB_BRANCH || 'main') {
  if (!owner || !process.env.GITHUB_TOKEN) {
    return { ok: false, error: 'GitHub credentials not configured (GITHUB_TOKEN, GITHUB_OWNER).' };
  }

  const targetRepo = repo || 'test';
  const url = `https://api.github.com/repos/${owner}/${targetRepo}/contents/${filePath}?ref=${branch}`;

  try {
    const res = await fetch(url, { headers: getHeaders() });
    if (!res.ok) {
      // Try with lowercase repo name if original failed
      if (targetRepo !== targetRepo.toLowerCase()) {
        const lowerUrl = `https://api.github.com/repos/${owner}/${targetRepo.toLowerCase()}/contents/${filePath}?ref=${branch}`;
        const lowerRes = await fetch(lowerUrl, { headers: getHeaders() });
        if (lowerRes.ok) {
          const data = await lowerRes.json();
          const content = Buffer.from(data.content, 'base64').toString('utf8');
          return { ok: true, content, sha: data.sha, repo: targetRepo.toLowerCase() };
        }
      }
    ...
    const content = Buffer.from(data.content, 'base64').toString('utf8');
    return { ok: true, content, sha: data.sha, repo: targetRepo };
  } catch (err) {
    return { ok: false, error: err.message };
  }
}

async function commitFileToGitHub(filePath, newContent, message, sha = null, owner = process.env.GITHUB_OWNER, repo = process.env.GITHUB_REPO || 'test', branch = process.env.GITHUB_BRANCH || 'main') {
  if (!owner || !process.env.GITHUB_TOKEN) {
  ...
  if (!sha) {
    const fetched = await getFileFromGitHub(filePath, owner, targetRepo, branch);
    if (fetched.ok) {
      sha = fetched.sha;
    }
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (targetRepo !== targetRepo.toLowerCase()) {
        const lowerUrl = `https://api.github.com/repos/${owner}/${targetRepo.toLowerCase()}/contents/${filePath}`;
